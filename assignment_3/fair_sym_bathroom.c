...
void* men_working(void * arg){
	...
	while(temp_runs > 0){
		...
		// Pass the baton to same gender in queue
		if (m_waiting > 0){
			m_waiting--;
			sem_post(&m_queue);
		}
		...
		// pass the baton to other gender when all are done
...
void* women_working(void * arg){
	...
	while(temp_runs > 0){
		...
		//passing baton to same gender while using
		if (w_waiting > 0){
			w_waiting--;
			sem_post(&w_queue);
		}
		...
		// Passong the baton to other gender if waiting
